/**
 * insole-utils（圧力データの検証・キャリブレーション・CoP・接地検出）のユニットテスト。
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import type { InsoleContactDownEvent, InsoleContactUpEvent } from '../../src/insole-utils.ts';
import {
  ContactDetector,
  MAX_UINT16,
  OrpheInsoleUtils,
  ...
  validatePress,
} from '../../src/insole-utils.ts';

test('validatePress: 正常系', () => {
...
test('SENSOR_LAYOUT_IMAGE / SENSOR_LAYOUT / mirrorForSide', () => {
  // 画像座標のゴールデン値
  assert.deepEqual(
...
test('computeCoP', () => {
  ...
  const single = new Array<number>(6).fill(0);
  single[0] = 1000;
...
test('ContactDetector', () => {
  ...
  const up = detector.update(300, 40) as InsoleContactUpEvent;
  assert.equal(up.event, 'up');
  assert.equal(up.stanceMs, 30, 'stance = 40 - 10');
  assert.equal(detector.isContact, false);
  // 再接地で flight 時間が返る
  const down2 = detector.update(1000, 100) as InsoleContactDownEvent;
  ...
  detector.reset();
  assert.equal(detector.isContact, false);
  assert.equal((detector.update(900, 0) as InsoleContactDownEvent).flightMs, null, 'no previous change after reset');
});

test('StuckChannelMonitor', () => {
...
test('OrpheInsoleUtils は名前付き export と同じメンバーをまとめる', () => {
  assert.deepEqual(OrpheInsoleUtils, {
    SENSOR_COUNT,
    MAX_UINT16,
    SENSOR_LAYOUT_IMAGE,
    SENSOR_LAYOUT,
  ...
  assert.equal(OrpheInsoleUtils.SENSOR_LAYOUT, SENSOR_LAYOUT);
  assert.equal(OrpheInsoleUtils.PressureCalibrator, PressureCalibrator);
});
